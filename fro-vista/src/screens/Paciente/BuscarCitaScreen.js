...
export default function BuscarCitaScreen() {
  ...
  const buscarDisponibilidad = async () => {
    if (!especialidadId || !tipoSede || !fechaSeleccionada) {
      Alert.alert(
        'Error',
        'Debe seleccionar especialidad, modalidad y fecha.'
      );
      return;
    }
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <Text>
              Bloque disponible: {item.hora_inicio} -{' '}
              {item.hora_fin}
            </Text>
          </TouchableOpacity>
        )}
