...
CU06 = dict(id='CU06', nombre='Solicitando restablecimiento de credenciales', actores=TRES,
  participantes=[ACTOR, P(V, 'vista'), *BASE, P('Usuario', 'tabla'), P('C_API_Adapter'), P('C_API_Brevo')],
  principal=[
    ...
    *upd('Usuario', 'UPDATE Usuario SET otp_codigo = ?, otp_expiracion = ? WHERE usuario_id = ?'),
    ...
    f'{API} -> C_API_Adapter: enviar_correo(email, codigo_otp)',
    f'C_API_Adapter -> C_API_Brevo: POST /v3/smtp/email',
    f'C_API_Brevo --> C_API_Adapter: return (HTTP 201 Aceptado)',
    f'C_API_Adapter --> {API}: return (correo aceptado)',
...
CU10 = dict(id='CU10', nombre='Administrando catálogo de perfil profesional', actores=['Profesional'],
  participantes=[ACTOR, P(V, 'vista'), *BASE, P('Profesional', 'tabla'), P('Bitacora_Auditoria', 'tabla'), P('C_API_Adapter'), P('C_API_Cloudinary')],
  principal=[
    ...
    *q('Profesional', 'SELECT foto_url, resena_curricular, areas_experticia, tipo_sede FROM Profesional WHERE usuario_id = ?', '1 registro'),
    ...
    f'{API} -> C_API_Adapter: subir_imagen(buffer, recorte 600x600)',
    f'C_API_Adapter -> C_API_Cloudinary: POST /image/upload',
    f'C_API_Cloudinary --> C_API_Adapter: return (secure_url)',
    f'C_API_Adapter --> {API}: return (secure_url)',
...
# ───────────────────────────── CU79 ─────────────────────────────
# Componentes segun el Diagrama de Componentes del Incremento 1:
# Cliente Movil (vistas), API REST, Seguridad JWT/BCrypt, Capa de Acceso a
# Datos y MySQL. El panel que se renderiza depende del rol autenticado.
VLOGIN = 'V_Inicio_Sesion'
SEG = 'C_Seguridad_JWT_BCrypt'
PANEL = {'Paciente': 'V_Inicio_Paciente', 'Profesional': 'V_Gestion_Profesional', 'Administrador': 'V_Gestion_Parametros'}
CU79 = dict(id='CU79', nombre='Segregando interfaces según rol de acceso', actores=TRES,
  vistas={rol: {'V': VLOGIN, 'V2': PANEL[rol]} for rol in TRES},
  participantes=[ACTOR, P('V', 'vista'), P('V2', 'vista'), P(SEG), *BASE,
                 P('Usuario', 'tabla'), P('Sesion_Usuario', 'tabla'), P('Bitacora_Auditoria', 'tabla')],
  principal=[
    ...
    *ins('Sesion_Usuario', 'INSERT INTO Sesion_Usuario VALUES (...)'),
    ...
    'V -> V2: abrir_el_panel_del_rol()',
    'V2 --> A: renderizar_panel_{A}()',
    'A -> V2: navegar_a_modulo(ruta habilitada)',
    f'V2 -> {API}: GET /recurso (Authorization: Bearer token)',
    ...
    *q('Sesion_Usuario', 'SELECT activa FROM Sesion_Usuario WHERE jti = ?', 'activa = TRUE'),
    ...
    f'{API} --> V2: return (HTTP 200 OK: datos del modulo)',
    'V2 --> A: mostrar_el_modulo_habilitado()',
  ],
  excepciones={
    1: dict(cortar='comparar_hashes', lineas=[
        '! Credenciales incorrectas',
        f'{SEG} --> {API}: return (credenciales invalidas)',
        f'{API} -> {DAO}: registrar_login_fallido(rut, ip)',
        *ins('Bitacora_Auditoria', "INSERT INTO Bitacora_Auditoria (accion = 'LOGIN_FALLIDO') VALUES (...)"),
        f'{DAO} --> {API}: return (Exito_Persistencia)',
        f'{API} --> V: return (HTTP 401: credenciales invalidas)',
        'V --> A: mostrar_el_error_de_credenciales()',
        'A -> V: reintentar_el_ingreso(rut, contrasena)'],
        reanudar='POST /auth/login'),
    2: dict(cortar='verificar_token_y_rol', lineas=[
        '! Token JWT adulterado: la firma no coincide',
        f'{SEG} --> {API}: return (firma invalida)',
        f'{API} -> {DAO}: revocar_la_sesion(jti)',
        *upd('Sesion_Usuario', 'UPDATE Sesion_Usuario SET activa = FALSE WHERE jti = ?'),
        f'{DAO} --> {API}: return (sesion revocada)',
        f'{API} --> V2: return (HTTP 401 TOKEN_INVALIDO)',
        'V2 ->> V2: limpiar_las_credenciales_locales()',
        'V2 --> A: forzar_el_cierre_de_sesion()',
        'A ->> A: contactar_a_soporte_tecnico()'],
        reanudar='ingresar_credenciales'),
    3: dict(cortar='verificar_token_y_rol', lineas=[
        '! El recurso esta reservado a otro rol',
        f'{SEG} --> {API}: return (rol no autorizado)',
        f'{API} -> {DAO}: registrar_bloqueo_rbac(usuario_id, ruta)',
        *ins('Bitacora_Auditoria', "INSERT INTO Bitacora_Auditoria (accion = 'BLOQUEO_ACCESO_RBAC') VALUES (...)"),
        f'{DAO} --> {API}: return (Exito_Persistencia)',
        f'{API} --> V2: return (HTTP 403: privilegios insuficientes)',
        'V2 --> A: redirigir_al_panel_de_inicio()'],
        reanudar='navegar_a_modulo'),
    4: dict(cortar='SELECT activa FROM Sesion_Usuario', lineas=[
        f'Sesion_Usuario --> {SQL}: return (activa = FALSE)',
        f'{SQL} --> {DAO}: return (resultado)',
        f'{DAO} --> {API}: return (sesion revocada o token expirado)',
        '! Token expirado o sesion cerrada desde otro dispositivo',
        f'{API} --> V2: return (HTTP 401 SESION_REVOCADA)',
        'V2 ->> V2: eliminar_el_token_local()',
        'V2 --> A: cerrar_la_sesion_con_aviso()',
        'A -> V: ingresar_credenciales(rut, contrasena)'],
        reanudar='POST /auth/login'),
  })
